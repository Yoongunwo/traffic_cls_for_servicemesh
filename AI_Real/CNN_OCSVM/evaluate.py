...
def main():
    ...
    #     torch.load(f'./AI_Real/CNN_OCSVM/Model/cnn_contrastive_encoder_{TEMPERATURE}_{EPOCHS}.pth') 
    # )
    model = Encoder_4x128().to(DEVICE)
    model.load_state_dict(
        torch.load(f'./AI_Real/CNN_OCSVM/Model/cnn_deep_contrastive_encoder_0.1_10_4x128.pth') 
    )
    model.eval()
