...
def train_draem(train_paths):
    ...
    torch.save(ae.state_dict(), f"./AI_Real/DRAEM/Model/ae_k8s_{EPOCHS}.pth")
    torch.save(clf.state_dict(), f"./AI_Real/DRAEM/Model/classifier_k8s_{EPOCHS}.pth")
    return ae, clf
...
if __name__ == "__main__":
    ae = DenoisingAE().to(DEVICE)
    clf = ResidualClassifier().to(DEVICE)
    ae.load_state_dict(torch.load(f"./AI_Real/DRAEM/Model/ae_k8s_{EPOCHS}.pth"))
    clf.load_state_dict(torch.load(f"./AI_Real/DRAEM/Model/classifier_k8s_{EPOCHS}.pth"))
