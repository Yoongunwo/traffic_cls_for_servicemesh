...
VEC_LEN = 1479
WIN_SIZE = 15
LIB_PATH = os.path.abspath("./v1/packet_parser_stack.so")
...
assert c_parser.init_session_storage() == 0, "C session storage init failed"

# === Preprocessing TorchScript ===
@torch.jit.script
def preprocess(flat: torch.Tensor) -> torch.Tensor:
    padded = torch.nn.functional.pad(flat, (0, 34 * 44 - flat.shape[0]))
    return padded[:34 * 44].reshape(1, 1, 34, 44)

# === Inference Worker ===
def inference_worker(q):
    DEVICE = torch.device("cpu")
    model = torch.jit.load('./Model/student_encoder_ts.pt').to(torch.float16)
    model.eval()
    ...
    while True:
        try:
            ...
            if arr_bytes is None:
                break
            stacked = np.frombuffer(arr_bytes, dtype=np.float32).reshape(WIN_SIZE, VEC_LEN)
            stacked_tensor = torch.tensor(stacked, dtype=torch.float32) / 255.0
            flat = stacked_tensor.flatten()
            img = preprocess(flat).to(dtype=torch.float16, device=DEVICE)
...
# === Proxy Class ===
class Proxy:
    def __init__(self, pause_event, TARGET_PORT, PROXY_PORT, POD_IP, q):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pod-ip', type=str, default='172.16.184.243')
    args = parser.parse_args()

    pause_event = asyncio.Event()
    q = queue.Queue()

    infer_thread = threading.Thread(target=inference_worker, args=(q,), daemon=True)
    infer_thread.start()
