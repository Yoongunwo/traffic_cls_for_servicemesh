...
float*** session_buffers = NULL;
int session_counts[MAX_SESSIONS] = {0};

void init_buffers() {
    session_buffers = (float***)malloc(MAX_SESSIONS * sizeof(float**));
    for (int i = 0; i < MAX_SESSIONS; ++i) {
        session_buffers[i] = (float**)malloc(WIN_SIZE * sizeof(float*));
        for (int j = 0; j < WIN_SIZE; ++j) {
            session_buffers[i][j] = (float*)malloc(VEC_LEN * sizeof(float));
        }
    }
}
...
void free_buffers() {
    for (int i = 0; i < MAX_SESSIONS; ++i) {
        for (int j = 0; j < WIN_SIZE; ++j) {
            free(session_buffers[i][j]);
        }
        free(session_buffers[i]);
    }
