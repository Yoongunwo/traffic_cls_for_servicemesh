...
class StatisticalChannelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        stat = torch.stack([
            stack.mean(dim=0),
            stack.std(dim=0)
        ], dim=0)  # [2, H, W] only mean/std
        return stat, self.labels[idx + self.window_size - 1]

# ✅ MAE 모델
class MAE(nn.Module):
    def __init__(self, in_ch=2, embed_dim=64, patch_size=4, img_size=16, mask_ratio=0.75):
        super().__init__()
        ...
        self.decoder = nn.Linear(embed_dim, patch_size * patch_size * in_ch)
...
def run_mae_pipeline():
    print(f"[MAE] Window size: {WIN}, Data: {ROOT}")
    ...
    torch.save(model.state_dict(), f'./AI/Statistic/MAE/Model/mae_model_{WIN}.pth')
    evaluate(model, test_loader, device)
